...
bool axi_check_raddr_fire(const axi_channel &axi) {
  if (axi.ar.valid && axi.ar.ready) {
    ...
    fprintf(stderr, "axi ar channel fired addr = 0x%lx, id = %d\n", axi.ar.addr, axi.ar.id);
#endif
    return true;
  }
  return false;
...
void axi_put_rdata(axi_channel &axi, const void *src, size_t n, bool last, uint8_t id) {
  n = 8; // = bus width (in bytes)
...
#ifdef DEBUG_LOG_AXI4
    fprintf(stderr, "axi r channel fired data = %lx, id = %d\n", axi.r.data[0], axi.r.id);
#endif
    return true;
  }
  return false;
...
#ifdef DEBUG_LOG_AXI4
    fprintf(stderr, "axi aw channel fired\n");
#endif
    return true;
  }
  return false;
...
#ifdef DEBUG_LOG_AXI4
    fprintf(stderr, "axi w channel fired\n");
#endif
    return true;
  }
  return false;
...
#ifdef DEBUG_LOG_AXI4
    fprintf(stderr, "axi b channel fired\n");
#endif
    return true;
  }
  return false;
